...
const Layout: PagicLayout = ({ config, title, outputPath, content, head, script, footer }) => {
  const isIndexPage = outputPath === 'index.html';
  const pageTitle = title ? (isIndexPage ? title : `${title} · ${config.title}`) : config.title;
  return (
    <html>
      <head>
        <meta charSet="utf-8" />
